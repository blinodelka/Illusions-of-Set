...
generated quantities {
  vector[n_test] pred;
  for(i in 1:n_test) {
    pred[i] = bernoulli_rng((likelihoods_test[i] * (1 - ((1-c)/(1 - c + c * num_trials_test[i]))))/((likelihoods_test[i] * (1 - ((1-c)/(1 - c + c * num_trials_test[i])))) + exp(normal_lpdf(0 | mu_other[i], sigma)) * (1-c)/(1 - c + c * num_trials_test[i])));}
}
...
for (k in 1:40) {
  ...
  indexes_assim_train = sample(indexes_assim, 82)
  indexes_contr_train = sample(indexes_contr, 82)
