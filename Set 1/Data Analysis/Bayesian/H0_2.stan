data {
	...
	vector[nY] size_diff; // разница между стимулами, не понадобится в этом анализе
	vector[nY] num_of_trials; // целевой предиктор, количество установочных проб
...
model {
  ...
  beta ~ normal(beta_mu, beta_sd);
	...
	for(i in 1:nY){
    Y[i] ~ bernoulli_logit(intercept + beta[Subj[i]]*num_of_trials[i]);
	}
}