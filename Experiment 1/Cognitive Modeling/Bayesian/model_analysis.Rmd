...
fit_full_crp_2 <- stan(model_code = model_full_crp_2, data = list_full_crp_2, warmup = 3000, iter = 15000)
```
...
```{r}
ggplot(data_with_predictions[data_with_predictions$answer.keys == 1], aes(x=size)) + geom_histogram(binwidth = 1, fill = "grey", color = "black") + xlab("difference between adaptors (in individual differential thresholds)") + ylab("observations")

ggplot(data_with_predictions[data_with_predictions$crp_full < 0.5], aes(x=size)) + geom_histogram(binwidth = 1, fill = "grey", color = "black") + xlab("difference between adaptors (in individual differential thresholds)") + ylab("predictions")

ggplot(data_with_predictions[data_with_predictions$blog_regression < 0.5], aes(x=size)) + geom_histogram(binwidth = 1, fill = "grey", color = "black") + xlab("difference between adaptors (in individual differential thresholds)") + ylab("predictions") + xlim(0.5,3.5)
```


```{r}
ggplot(data_with_predictions[data_with_predictions$answer.keys == 0], aes(x=size)) + geom_histogram(binwidth = 1, fill = "grey", color = "black") + xlab("difference between adaptors (in individual differential thresholds)") + ylab("observations")
...
ggplot(data_with_predictions[data_with_predictions$crp_full > 0.5], aes(x=number_of_fixational_trials)) + geom_histogram(binwidth = 1, fill = "grey", color = "black") + xlab("difference between adaptors (in individual differential thresholds)") + ylab("predictions") + xlim(0.5, 8.5)

ggplot(data_with_predictions[data_with_predictions$blog_regression > 0.5], aes(x=number_of_fixational_trials)) + geom_histogram(binwidth = 1, fill = "grey", color = "black") + xlab("difference between adaptors (in individual differential thresholds)") + ylab("predictions") + xlim(0.5,8.5)
```
...
```{r}
true_categories <- 1 - data_for_stan$answer.keys
...
# Recall-Precision curve             
RP.perf_lr <- performance(pred_r, "prec", "rec")
...
sum(data_with_predictions$blog_regression < 0.5 & list_for_stan$Y == 1)/sum(list_for_stan$Y == 1)
sum(data_with_predictions$blog_regression > 0.5 & list_for_stan$Y == 0)/sum(list_for_stan$Y == 0)
(sum(data_with_predictions$blog_regression < 0.5 & list_for_stan$Y == 1) + sum(data_with_predictions$blog_regression > 0.5 & list_for_stan$Y == 0))/length(list_for_stan$Y)

# Model
sum(data_with_predictions$crp_full < 0.5 & list_for_stan$Y == 1)/sum(list_for_stan$Y == 1)
sum(data_with_predictions$crp_full > 0.5 & list_for_stan$Y == 0)/sum(list_for_stan$Y == 0)
(sum(data_with_predictions$crp_full < 0.5 & list_for_stan$Y == 1) + sum(data_with_predictions$crp_full > 0.5 & list_for_stan$Y == 0))/length(list_for_stan$Y)
```
...
for (k in 1:50) {
  ...
  # LOG REGRESSION
  data_for_regression = data_for_stan
  # undersampling
